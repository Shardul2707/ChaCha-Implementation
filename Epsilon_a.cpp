...
// Function to zero specified bit positions (across 512-bit state)
void zero_selected_bits(uint32_t state[16]) {
    const int zero_bits[] = {
        2,3,7,8,9,10,11,12,13,14,15,19,20,21,22,26,27,28,29,30,
        38,47,63,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,
        83,84,85,86,87,88,89,90,95,96,97,98,99,103,104,105,106,107,
        108,109,110,111,112,113,114,115,116,117,118,119,120,121,122,
        123,124,125,126,127,128,129,130,131,136,137,138,139,140,141,
        142,143,144,145,146,147,148,149,150,151,152,153,154,155,156,
        157,158,159,160,161,162,163,167,175,179,180,181,182,187,188,
        189,190,191,200,223,224,225,226,227,228,229,230,231,232,236,
        237,238,239,240,241,242,243,248,249,250,251,252,253,254,255
    };
    const int zero_bits_count = sizeof(zero_bits) / sizeof(zero_bits[0]);

    for (int i = 0; i < zero_bits_count; ++i) {
        int pos = zero_bits[i];
        int word = pos / 32;
        int bit  = pos % 32;
        state[word+4] &= ~(1U << bit); // Force bit to 0
    }
}
...
int main() {
    ...
    uint64_t n = 1024; // Number of trials
    ...
    //    for (int I = 0; I < 32; I++) {
            ...
            for (uint64_t t = 0; t < n; ++t) {
                ...
                // zero_selected_bits(Ini);
                // zero_selected_bits(X_prime);
            ...
            double epsilon_a = (2.0 * prob) - 1.0;
            // if(epsilon_a >= 0.6 || epsilon_a <= -0.6) {
                std::cout << "prob=" << prob << std::endl;
                //std::printf("f=%2d, I=%2d, Bit=%2d, epsilon_a=%.6f\n", f, I, (f-4)*32 + I, epsilon_a);
                // if (t == 1023 ||t == 32767 || t == 1048575 || t == 33554431 || t == 1073741823)
                // {
                    std::cout << "n=" << n << ", epsilon_a=" << epsilon_a << std::endl;
                // }
            //std::cout<<zero_bits_count<<std::endl;
            //    }
    //}
